...
MAX_WORKERS = 20  # Increased workers for global processing
...
def main() -> None:
    ...
    # Deduplicate trials across all targets for efficient processing
    unique_trials = {}  # trial_id -> trial_config
    ...
    print(f"Processing {total_unique} unique trials across {len(targets)} targets")

    # Process all unique trials once
    processed_results = {}  # trial_id -> (report_item, raw_data)

    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_id = {
            executor.submit(process_trial, trial_config, "Global"): tid
            for tid, trial_config in unique_trials.items()
        }
        ...
        try:
            for future in as_completed(future_to_id, timeout=TOTAL_TIMEOUT):
                ...
                try:
                    ...
                    processed_results[tid] = (report, raw)
                    print(f"[{completed_count}/{total_unique}] Processed {tid}")
                except TimeoutError:
                    print(f"[{completed_count}/{total_unique}] Timeout processing {tid}, skipping")
                except Exception as e:
                    print(f"[{completed_count}/{total_unique}] Error processing {tid}: {e}")
        except TimeoutError:
            print(f"\n⚠ Global processing timed out after {TOTAL_TIMEOUT}s")

    # Map results back to targets and save
    target_summaries = []
    all_reports = []
    all_raw = []
    ...
    for target in targets:
        ...
        for trial in target.get("trials", []):
            ...
            if tid in processed_results:
                report_orig, raw_orig = processed_results[tid]
                if report_orig and raw_orig:
                    # Create target-specific copies
        ...
        target_summaries.append({
            ...
            "trial_count": len(target.get("trials", [])),
