    ...
    if HAS_REQUESTS:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Error fetching data for term {query_term}: {response.status_code}")
                return []
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as response:
                if response.status == 200:
                    ...
                    print(f"Error fetching data for term {query_term} (urllib): {response.status}")
                    return []
        except Exception as e:
            print(f"Exception fetching data for term {query_term} (urllib): {e}")
            return []

def extract_trials(api_studies):
...
def main():
    ...
    for target in targets:
        ...
        existing_target = next((t for t in targets if t['name'].lower() == target_name.lower()), None)
        existing_ids = {t['id'] for t in existing_target.get('trials', [])} if existing_target else set()
        ...
        updated_target = next((t for t in data['targets'] if t['name'].lower() == target_name.lower()), None)
        updated_ids = {t['id'] for t in updated_target.get('trials', [])} if updated_target else set()
